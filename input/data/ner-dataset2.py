...
for file_name in file_name_list:
    ...
    with open(file_path_name,'r',encoding='utf-8') as f:
        ...
        for line in f:
            ...
            if start_str in line:  # already get a set of que-tri-ans, start handle
                # get 1st entity in triple
                entities = t_str.split("|||")[0].split(">")[1].strip()
                q_str = q_str.split(">")[1].replace(" ", "").strip()
                
                if entities in q_str:  # entity in triple is in question
                    q_list = list(q_str)  # string -> list of chars
